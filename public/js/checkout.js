...
async function init(){
  ...
  if(!el)return;
  if(ESH.cart.count()===0){
    el.innerHTML='<div class="glass checkout-panel text-center"><h3>Your cart is empty</h3><p class="text-muted mt-1">Add some products before checking out.</p><a href="/products.html" class="btn btn-primary mt-3">Browse products</a></div>';
    document.getElementById("summaryCard").innerHTML="";
    return;
  }
  S.needsShipping=ESH.cart.hasPhysical();
  S.step=1;
  var a=await ESH.api("/api/payment-methods");
  if(a.ok)S.methods=a.data.methods;
  var b=await ESH.api("/api/wallets/public");
  if(b.ok)S.wallets=b.data.wallets.filter(function(w){return w.network==="BTC"||w.network==="LTC";});
  render();
}

function renderSteps(){
  var steps=["Cart"];
  if(S.needsShipping)steps.push("Shipping");
  steps.push("Payment");
  var h='<div class="checkout-steps">';
  for(var i=0;i<steps.length;i++){
    var n=i+1;
    var cls=n<S.step?"done":n===S.step?"active":"";
    h+='<div class="checkout-step '+cls+'"><span class="num">'+(n<S.step?"&#10003;":n)+'</span>'+steps[i]+'</div>';
  }
  h+='</div>';
  return h;
}
...
function renderCart(){
  ...
  var h='<div class="glass checkout-panel"><h3>Your cart</h3>';
  for(var i=0;i<items.length;i++){
    var it=items[i];
    h+='<div class="flex justify-between items-center" style="padding:0.8rem 0;border-bottom:1px solid var(--border-soft);"><div><div style="font-weight:600;">'+esc(it.title)+'</div><div class="text-muted" style="font-size:0.8rem;">'+(it.type==="digital"?"Digital":"Physical")+" &middot; "+ESH.formatPrice(it.price)+'</div></div><div class="flex items-center gap-1"><input type="number" min="1" value="'+it.qty+'" data-qty="'+it.product_id+'" style="width:60px;text-align:center;padding:0.4rem;"><button class="icon-btn" data-remove="'+it.product_id+'" title="Remove">&times;</button></div></div>';
  }
  h+='<div class="flex justify-between mt-2"><a href="/products.html" class="btn btn-ghost btn-sm">&larr; Continue shopping</a><button class="btn btn-primary" id="btnNext">Continue</button></div></div>';
  return h;
}

function renderShipping(){
  var s=S.shipping;
  var h='<div class="glass checkout-panel"><h3>Shipping address</h3>';
...
function renderPayment(){
  var opts=["paypal_manual","revolut_manual","crypto_manual"];
  var available=[];
  for(var i=0;i<opts.length;i++){
    var found=false;
    for(var j=0;j<S.methods.length;j++){if(S.methods[j].code===opts[i]){found=true;break;}}
    if(found||S.methods.length===0)available.push(opts[i]);
  }
  var detail="";
  if(S.payment_method==="crypto_manual"){
    ...
    for(var i=0;i<S.wallets.length;i++){
      var w=S.wallets[i];
      detail+='<div class="wallet-option '+(S.wallet_id===w.id?"selected":"")+'" data-wallet="'+w.id+'"><div><strong>'+(NET[w.network]||w.network)+'</strong><div class="net">Min. '+ESH.formatPrice(w.min_amount)+'</div></div><span class="badge badge-blue">'+w.network+'</span></div>';
    }
    if(S.wallets.length===0)detail+='<p class="text-muted">No wallets configured.</p>';
    detail+='</div>';
  }else if(S.payment_method){
    detail='<div class="pay-detail">'+(DESC[S.payment_method]||"")+'</div>';
  }
  ...
  for(var i=0;i<available.length;i++){
    var c=available[i];
    h+='<div class="pay-method '+(S.payment_method===c?"selected":"")+'" data-method="'+c+'">'+(ICONS[c]||"")+'<span>'+(LABELS[c]||c)+'</span>'+(c==="crypto_manual"?"<small>BTC &middot; LTC</small>":"")+'</div>';
  }
  h+='</div><div id="payDetail">'+detail+'</div>';
  h+='<div class="mt-2"><div class="coupon-row"><input id="couponInput" placeholder="Discount code" value="'+esc(S.coupon_code)+'"><button class="btn btn-ghost" id="applyCoupon">Apply</button></div>';
  if(S.couponError)h+='<div class="alert alert-danger mt-1">'+S.couponError+'</div>';
  if(S.discount>0)h+='<div class="alert alert-success mt-1">Discount: -'+ESH.formatPrice(S.discount)+'</div>';
  h+='</div><div class="flex justify-between mt-3"><button class="btn btn-ghost btn-sm" id="btnBack">&larr; Back</button><button class="btn btn-primary" id="confirmOrder" '+(S.payment_method?"":"disabled")+'>Confirm & pay</button></div></div>';
  return h;
}

function renderSummary(){
  var items=ESH.cart.get();
  var sub=ESH.cart.subtotal();
  var ship=(S.needsShipping&&!S.freeShipping)?6.9:0;
  var tax=+((sub-S.discount)*0.22).toFixed(2);
  var tot=+(sub-S.discount+tax+ship).toFixed(2);
  ...
  h+='<div class="summary-item"><span>Shipping</span><span>'+(ship?ESH.formatPrice(ship):"Free")+'</span></div>';
  h+='<div class="summary-item"><span>VAT (22%)</span><span>'+ESH.formatPrice(tax)+'</span></div>';
  h+='<div class="summary-item total"><span>Total</span><span>'+ESH.formatPrice(tot)+'</span></div>';
  h+='<div class="trust-row">Secure payment &mdash; Manual verification</div></div>';
...
function bindEvents(){
  ...
  if(btnNext)btnNext.onclick=function(){
    if(S.step===1){
      if(S.needsShipping){S.step=2;}
      else{S.step=3;}
    }else if(S.step===2&&S.needsShipping){
      S.shipping={line1:val("line1"),line2:val("line2"),city:val("city"),state:val("state"),postal_code:val("postal_code"),country:val("country")};
      var em=document.getElementById("guestEmail");if(em)S.guest_email=em.value;
      if(!S.shipping.line1||!S.shipping.city||!S.shipping.postal_code){ESH.toast("Please fill all required fields.","error");return;}
      S.step=3;
    }
    render();
  };
  var btnBack=document.getElementById("btnBack");
  if(btnBack)btnBack.onclick=function(){S.step=Math.max(1,S.step-1);render();};
  ...
  if(applyBtn)applyBtn.onclick=async function(){
    var code=document.getElementById("couponInput").value.trim();S.coupon_code=code;
    if(!code){S.discount=0;S.freeShipping=false;S.couponError=null;return render();}
    var r=await ESH.api("/api/coupons/preview",{method:"POST",body:{code:code,subtotal:ESH.cart.subtotal()}});
    if(!r.ok){S.discount=0;S.freeShipping=false;S.couponError=r.error;}else{S.discount=r.data.discount;S.freeShipping=r.data.free_shipping;S.couponError=null;}
    render();
  };
...
async function submitOrder(){
  ...
  var r=await ESH.api("/api/checkout",{method:"POST",body:{items:items,payment_method:S.payment_method,wallet_id:S.payment_method==="crypto_manual"?S.wallet_id:null,coupon_code:S.coupon_code||null,guest_email:S.guest_email||undefined,shipping_address:S.needsShipping?S.shipping:undefined}});
  if(!r.ok){ESH.toast(r.error,"error");btn.disabled=false;btn.textContent="Confirm & pay";return;}
  ESH.cart.clear();
