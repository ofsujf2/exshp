...
function register(router) {
  ...
  router.put('/api/admin/orders/:id', 'admin', async (req, res, params) => {
    ...
    if (!wasPaid && fields.payment_status === 'paid') {
      ...
      for (const it of items) {
        ...
        if (product && product.type === 'digital') fulfillDigitalItem(product, order);
      }
    }
  ...
  router.get('/api/admin/gateways', 'admin', async (req, res) => {
    const rows = db.prepare('SELECT id, code, name, public_key, status FROM payment_gateways ORDER BY id ASC').all();
    sendJson(res, 200, { gateways: rows }); // le secret_key non vengono mai restituite al client
  });
